/*---------------------------------------------------------------------
 * @file     zf_device_imu.cpp
 * @brief    IMU惯性测量单元驱动实现文件
 * @date     2026/01/09
 *---------------------------------------------------------------------
 * @author   Seekfree
 *---------------------------------------------------------------------
 * @brief    IMU惯性测量单元驱动类实现文件
 *           包含设备初始化、型号识别、加速度计、陀螺仪、磁力计数据采集等功能的实现
 *---------------------------------------------------------------------
 */
...
imu_device_type_enum zf_device_imu::init(void)
{
    ...
    if(fd_event < 0)
    {
        std::cerr << "IMU打开事件文件失败: " << IMU_EVENT_PATH << ", errno:" << errno << std::endl;
    ...
    {
        std::cerr << "IMU写入事件文件失败, errno:" << errno << std::endl;
        close(fd_event);
        imu_type = DEV_NO_FIND;
        return imu_type;
    }
    ...
    if(read_val == DEV_IMU660RA || read_val == DEV_IMU660RB || read_val == DEV_IMU963RA)
    {
        ...
        {
            case DEV_IMU660RA:
                std::cout << "IMU初始化成功, 型号: " << imu_type << " (DEV_IMU660RA)" << std::endl;
                break;
            case DEV_IMU660RB:
                std::cout << "IMU初始化成功, 型号: " << imu_type << " (DEV_IMU660RB)" << std::endl;
                break;
            case DEV_IMU963RA:
                std::cout << "IMU初始化成功, 型号: " << imu_type << " (DEV_IMU963RA)" << std::endl;
                break;
            default:
                std::cout << "IMU初始化成功, 型号: " << imu_type << " (未知)" << std::endl;
                break;
        }
    }
    else
    {
        std::cerr << "IMU读取到无效型号值: " << read_val << ", 初始化失败" << std::endl;
        imu_type = DEV_NO_FIND;
        return imu_type;
    }
    ...
    if(imu_type == DEV_IMU660RA || imu_type == DEV_IMU660RB)
    {
        ...
        {
            std::cerr << "IMU打开加速度/陀螺仪文件失败" << std::endl;
            imu_close_all_fd();
            imu_type = DEV_NO_FIND;
        }
    }
    else if(imu_type == DEV_IMU963RA)
    {
        ...
        {
            std::cerr << "IMU打开所有传感器文件失败" << std::endl;
            imu_close_all_fd();
            imu_type = DEV_NO_FIND;
        }
    }
